...
export function parseEventLine(line: string, prevPosition?: Position): ParseResult<KifEvent>{
    ...
    const moveNumber = parseInt(numberText, 10)
    ...
    const { position, piece, drop } = moveTextResult.value
    const from = fromResult.value
    return { ok: true, value: { type: "move", moveNumber, player, piece, moveText, position, from, drop}}
