...
export function useKifPlayerOrig(library: KifEntry[], navigation: ReturnType<typeof useKifNavigation>) {
  ...
  function playFirst(list: KifEntry[]) {
    if (list.length === 0) return;
    playByEntryId(list[0].id);
  }

  function playLast(list: KifEntry[]) {
    if (list.length === 0) return;
    playByEntryId(list[list.length - 1].id);
  }


  function playNext(list: KifEntry[]) {
  ...
  function playPrev(list: KifEntry[]) {
    if (!currentEntryId) return;
